...
def test(args,test_loader,new_data,device):
    state_dict = torch.load(args.checkpoint)
    input_dim = 1      # 数据的特征数
    hidden_dim = 50    # 隐藏层的神经元个数
    num_layers = 2     # LSTM的层数
    output_dim = 1     # 预测值的特征数
    model_new = LSTM(args,input_size=input_dim, hidden_size=hidden_dim, num_layers=num_layers).to(device)
...
if __name__ == '__main__':
    ...
    num_layers = 2     # LSTM的层数
    output_dim = 1     # 预测值的特征数
    model = LSTM(args,input_size=input_dim, hidden_size=hidden_dim, num_layers=num_layers).to(device)
    ...
    test(args,test_loader,new_data,device)
